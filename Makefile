...
npm-install:
	...
	@npm install chalk@5 node-html-parser express
...
# ─── Launch WebUI ──────────────────────────────────────────────────────────────
web:
	@echo "Starting ENux AI WebUI on http://localhost:3000 ..."
	@$(NODE) server.mjs

.PHONY: all check-node install-ollama pull-model npm-install chmod run web update clean uninstall
